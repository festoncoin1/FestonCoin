...
*Setup instructions for a gitian build of FestonCoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the FestonCoin
...
- [Building FestonCoin](#building-feston)
...
Clone the git repositories for feston and gitian and then checkout the feston version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/feston-crypto/feston.git
cd feston
...
Follow the instructions in [doc/release-process.md](release-process.md) in the feston repository
...
To build FestonCoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the feston repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/feston/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/feston-crypto/feston
...
For example:
```bash
URL=https://github.com/crowning-/feston.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit feston=${COMMIT} --url feston=${URL} ../feston/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit feston=${COMMIT} --url feston=${URL} ../feston/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit feston=${COMMIT} --url feston=${URL} ../feston/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/feston-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/feston-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/feston-build.assert
```
...
[feston/gitian.sigs](https://github.com/feston-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
