...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::FTON);
    ...
    if (!settings.contains("nAnonymizeFTONAmount"))
        settings.setValue("nAnonymizeFTONAmount", 1000);

    nObfuscationRounds = settings.value("nObfuscationRounds").toLongLong();
    nAnonymizeFTONAmount = settings.value("nAnonymizeFTONAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeFTONAmount"))
        SoftSetArg("-anonymizefestonamount", settings.value("nAnonymizeFTONAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in feston.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeFTONAmount:
            return QVariant(nAnonymizeFTONAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case ObfuscationRounds:
            ...
            break;
        case AnonymizeFTONAmount:
            nAnonymizeFTONAmount = value.toInt();
            settings.setValue("nAnonymizeFTONAmount", nAnonymizeFTONAmount);
            emit anonymizeFTONAmountChanged(nAnonymizeFTONAmount);
            break;
        case CoinControlFeatures:
