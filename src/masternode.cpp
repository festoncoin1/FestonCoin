...
unsigned CMasternode::Level(CAmount vin_val, int blockHeight)
{
    ...
    } else if (blockHeight >= 3000 && blockHeight < 20000) {
      switch(vin_val) {
          case 1000 * COIN: return 1;
          case 1500 * COIN: return 2;
          case 3000 * COIN: return 3;
      }
    } else if (blockHeight >= 20000 && blockHeight < 50000) {
      switch(vin_val) {
...
*/
    ...
    // should be at least not earlier than block when 1000 FestonCoin tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && mi->second) {
        CBlockIndex* pMNIndex = mi->second;                                                        // block for 1000 FestonCoin tx -> 1 confirmation
