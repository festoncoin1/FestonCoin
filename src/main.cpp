...
#if defined(NDEBUG)
#error "FestonCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ufeston) are considered zero fee (for relaying and mining)
...
CAmount GetBlockValue(int nHeight)
{
  // anti instamine
    ...
    if ( nHeight == 0 ) {    // premine
      ...
      nSubsidy = 0.5 * COIN;
    } else if (nHeight > 3000 && nHeight <= 20000) {
      nSubsidy = 20 * COIN;
      ...
      nSubsidy = 200 * COIN;
    } else if (nHeight > 350000 && nHeight <= 550000) {
      nSubsidy = 400 * COIN;
    } else if (nHeight > 550000 && nHeight <= 850000) {
      nSubsidy = 200 * COIN;
    } else if (nHeight > 850000 && nHeight <= 1500000) {
      nSubsidy = 150 * COIN;
    } else if (nHeight > 1500000 && nHeight <= 3000000) {
      nSubsidy = 100 * COIN;
...
{
    if (nHeight <= Params().StartMNPaymentsBlock()) { // block 200
        return 0;
    } else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight > Params().StartMNPaymentsBlock()) { // height <= 5000 & height < 200
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake() && nHeight > Params().LAST_POW_BLOCK() && IsSporkActive(SPORK_9_ACCEPT_BLOCK_SPAM_FILTER_ENFORCEMENT) ) {
        LOCK(cs_main);
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
...

    {
        ...
        if (pfrom && GetBoolArg("-blockspamfilter", DEFAULT_BLOCK_SPAM_FILTER)) {
            ...
            if (chainActive.Height() > Params().LAST_POW_BLOCK() && IsSporkActive(SPORK_8_PROCESS_BLOCK_SPAM_FILTER_ENFORCEMENT)) {
